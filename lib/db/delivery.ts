...
export async function updateDeliveryStatusByToken(
  ...
  // COD capture-on-delivered — closes the #91 ledger loop. A delivery order is the
  // Egypt cash spine (collected at the door), so marking «delivered» captures the
  // expected cash (= the order's tool-computed total) into the COD ledger against
  // the assigned driver. Idempotent + null-tolerant; never blocks the status update.
  if (target === "delivered") {
    try {
      ...
      console.error("[delivery] COD capture-on-delivered error", e);
    }
  }
