...
test("R1-prep — the three future fact NAMES are declared, and unused by the predicate", () => {
  assert.deepEqual([...FUTURE_ACCEPTANCE_FACT_NAMES], [
    "customer_confirmed_at",
    "restaurant_accepted_at",
    "restaurant_accepted_by",
  ]);
  // Proof they touch no decision: passing them changes nothing.
  const withFacts = { orderStatus: "pending_confirmation" as OrderStatusKey, customer_confirmed_at: "2026-01-01T00:00:00Z", restaurant_accepted_at: "2026-01-01T00:00:00Z", restaurant_accepted_by: "m1" };
  assert.equal(isAcceptanceEligible(withFacts), false); // still false — facts ignored
});
