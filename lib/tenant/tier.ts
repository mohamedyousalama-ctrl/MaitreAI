...
// address_flow_v2 (WO-ADDR): written delivery addresses are matched against named
// tenant zones before any optional pin fallback. STRICT per-tenant switch, DEFAULT
// OFF, never implied by tier='pro'; flag-off keeps prompt/tools byte-identical.
...
// allergy_calm_hold (WO-CALM): deterministic allergy stop UX — when the existing gate/
...
// goal_logic_rule6_annotation_pivot (Week-2 orchestration): explicit-only pivot
// for residual read_not_understood deflections; flag OFF preserves deterministic
// goal_clarify while shadow-logging candidates, flag ON routes through the model
// with a no-fabrication annotation.
// call_count_observability (WO-S0-OBS): explicit-only measurement switch for per-turn
// model-call counts and shadow cap logs. Default OFF; never blocks or rewrites.
// perception_async (WO-S0-PERC): strict post-send async perception switch. OFF preserves
// the legacy synchronous goal_logic/perception path; ON queues a separately priced
// perception read after the WhatsApp reply dispatch resolves.
// typed_interactive_actions (WO-S0-TYPED): WhatsApp button/list taps whose ids are
...
// typed_quantity_fill (WO-QTY): after a pending quantity prompt for a known draft
// item, a bare numeric reply (digits / Arabic-Indic / Arabic number word 1..20)
// or a qty:N button id can set the quantity through the deterministic typed rail.
// STRICT per-tenant switch, DEFAULT OFF, never implied by tier='pro'.
// action_claim_guard (WO-ACTION-CLAIM-GUARD): a deterministic post-turn guard in respond.ts
// that blocks any reply CLAIMING a draft modification when no draft mutation actually happened
// that turn (no mutation tool ran AND the draft fingerprint is unchanged) — the "fabricated
// action" class («هخلي البيتزا واحدة ✅» with zero tool calls). It re-runs the model ONCE with a
// strict use-the-tools directive; if the retry still fabricates it sends a fixed honest clarify.
// STRICT per-tenant switch, DEFAULT OFF, never implied by tier='pro' — flag-off is byte-identical.
// finish_line (WO-FINISH-LINE): three deterministic finishers in respond.ts — (A) the order
// recap is RENDERED BY CODE from the priced draft (Arabic-Indic, complete, never truncated,
// zero model calls) instead of model prose; (B) a post-turn contract guarantees every reply
// carries a question, a recap, or a handoff (else appends the deterministic next-step, killing
