...
export const EGYPTIAN = [
  ...
  // WO-DIALECT-LIVE (29 Aug). Observed on PRODUCTION, on the najd demo tenant, in the
  // delivery-address turn: «تمام 👌 ابعت لي العنوان بالتفصيل». The linter returned
  // {"ok":true,"hits":[]} on that exact string. «ابعت» is the Egyptian/Levantine
  // imperative of «بعت»; a Riyadh speaker says «أرسل/ارسل لي» or «اكتب لي», so there is no
  // Najdi homograph to protect and the marker is safe to ban outright.
  //
  // NOTE ON SCOPE: this list is scanned against Khalid's OUTPUT only (customer-turn.ts
  // passes result.reply). «ابعت» also appears in lib/ai/order-confirm.ts, but there it is
  // an INPUT detector matching what a CUSTOMER may type — Egyptian input is legitimate and
  // must keep matching. Banning it here does not touch that path.
  "ابعت", "ابعتلي", "ابعتها", "ابعته", "هبعت", "نبعت",
  // Same family, same absence of a Najdi homograph.
  "عاوزة", "عايزين", "عاوزين", "مستني", "مستنية", "بحب", "بحبش",
