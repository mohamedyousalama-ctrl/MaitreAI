...
/** The legacy (flag-OFF) allergy block, in the tenant's dialect.
 *
 *  It took no dialect and was authored entirely in Cairene, so a SAUDI tenant's model was
 *  told to say «صحتك أهم حاجة عندنا» and — worse — to give the mandatory cross-contact
 *  caveat as «ما نقدرش نضمن عدم وجود أثر». That is the Egyptian ما...ش circumfix on a
 *  SAFETY utterance: the one sentence in the whole prompt that must land cleanly with a
 *  customer who has just said they have an allergy. The newer companionAllergyBlock()
 *  below is already clean Najdi; this block is what the demo tenant actually runs,
 *  because allergy_companion_mode is deliberately OFF. */
export function legacyAllergyBlock(dialect: string): string {
  const sa = dialect === "saudi";
  return "## Allergy safety — HIGH STAKES (a health matter, NEVER a guess)\nIf the customer mentions an allergy / «حساسية» / «عندي حساسية من…» / a medical dietary restriction (also via Franco «3andy 7asaseya» — decode it), treat it as a SAFETY matter, not a preference:\n0. ACKNOWLEDGE IT THE MOMENT IT'S MENTIONED — on the FIRST mention, in that SAME reply, name the specific allergen back to them (" + (sa ? "«تمام، خذت بالي إن عندك حساسية من السمك 🙏»" : "«تمام، خدت بالي إنك عندك حساسية من السمك 🙏»") + ") BEFORE you continue with the menu, the order, or anything else. NEVER skip past an allergy to ask " + (sa ? "«وش تحب تطلب؟»" : "«تحب تطلب إيه؟»") + " first. If they mentioned it alongside an order or a browse request, handle the ALLERGY acknowledgement first, then the order/menu. This includes Franco — DECODE it and name the allergen back: «3andy 7asaseya men el samak» = «عندي حساسية من السمك» → reply " + (sa ? "«خذت بالي إن عندك حساسية من السمك 🙏»" : "«خدت بالي إنك عندك حساسية من السمك 🙏»") + ". Capture the specific allergen the first time — never wait for a repeat, never reduce it to a vague " + (sa ? "«أساعدك تختار بأمان»" : "«هساعدك تختار بأمان»") + ".\n1. Take it seriously and warmly (" + (sa ? "«صحتك أهم شي عندنا 🙏»" : "«صحتك أهم حاجة عندنا 🙏»") + ") — never brush past it, never override or downplay it to make a sale («غالباً تمام» is forbidden), and never upsell an item that conflicts with the stated allergy. Safety beats the sale, ALWAYS.\n2. Check the specific item's «allergens» data in the menu below:\n   • allergens list INCLUDES the allergen → tell them honestly it's NOT suitable, and suggest a real alternative whose allergens list does NOT include it.\n   • item HAS an allergens list that does NOT include the allergen → you may say its listed allergens don't include it, BUT always add the honest cross-contact caveat " + (sa ? "(مطبخ مشترك، ما نقدر نضمن عدم وجود أثر)" : "(مطبخ مشترك، ما نقدرش نضمن عدم وجود أثر)") + " — NEVER an absolute " + (sa ? "«ما فيها أبداً»" : "«مفيهاش خالص»") + " / «آمن ١٠٠٪» guarantee.\n   • item shows NO «allergens» line at all, OR the data looks incomplete, OR you're unsure → DO NOT guess and DO NOT assume it's allergen-free. Escalate to the team/kitchen to verify (escalate_to_human). Uncertainty = escalate, never reassure.\n3. CARRY IT THROUGH the order: once a customer states an allergy, you MUST repeat it as a clear note in EVERY order readback/recap and in the final confirm — never omit it. Put it on its own line, e.g. «⚠️ ملاحظة مهمة: حساسية من المكسرات»، so the kitchen sees it before the order is confirmed.\nCRITICAL: absence of an «allergens» line for an item means its allergen data is UNKNOWN — it does NOT mean the item is free of that allergen. When unknown, escalate; never say it's safe.";
}
