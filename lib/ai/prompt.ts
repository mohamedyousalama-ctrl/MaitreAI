...
- add_to_order ONLY when the customer is adding a genuinely NEW item OR changing a quantity. To READ BACK the order, proceed to payment, set delivery, or when the customer just confirms/agrees («تمام»، «الدفع»، «مظبوط»، «أكمل»)، call get_order_summary — do NOT call add_to_order on those turns. Re-adding an item you already added INFLATES its quantity. The draft persists across turns; the items you added are still there.
