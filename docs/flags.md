...
| `memory_allergy_gate` | Memory allergy gate: when explicitly ON, customer-memory allergy notes are mapped deterministically and joined into the same allergen gate input when a current draft item carries that allergen in menu allergen/cross-contact data. No model call; OFF never reads `customer_memory` and keeps today's path byte-identical. Default OFF. |
...
| `goal_logic_rule6_annotation_pivot` | Week-2 orchestration annotation pivot for the residual rule-6 `read_not_understood` path only. **Strict explicit flag** (`isFeatureExplicitlyEnabled`; not implied by `tier='pro'`). OFF → deterministic `goal_clarify` behavior remains byte-identical, while `customer-turn.ts` best-effort shadow-logs `annotation_pivot_rule6_shadow` candidates for measurement. ON → the same rule-6 case routes through the normal model loop with a no-fabrication clarification annotation (`annotation_pivot_rule6_live`), inheriting the existing final banned-word scrub, numeral validator, and allergen checkpoint. Rules 1-5 and the already-safe rule-6 defer paths are unchanged. Default OFF. |
| `call_count_observability` | Per-turn model-call-count visibility and shadow caps (WO-S0-OBS): when explicitly ON, `customer-turn.ts` logs `turn_call_count`, persists `calls_used` plus cap-shadow metadata to `agent_runs.meta`, and emits a structured `cap_shadow_violation` log if calls or cost exceed the shadow thresholds. Observation only: no early exit, no truncation, no reply rewrite. Requires the prepare-only `0098` migration for durable `agent_runs.meta`; runtime update is best-effort until applied. Default OFF. |
