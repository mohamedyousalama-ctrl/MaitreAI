...
| `call_count_observability` | Per-turn model-call-count visibility and shadow caps (WO-S0-OBS): when explicitly ON, `customer-turn.ts` logs `turn_call_count`, persists `calls_used` plus cap-shadow metadata to `agent_runs.meta`, and emits a structured `cap_shadow_violation` log if calls or cost exceed the shadow thresholds. Observation only: no early exit, no truncation, no reply rewrite. Requires the prepare-only `0095` migration for durable `agent_runs.meta`; runtime update is best-effort until applied. Default OFF. |
