...
**Last updated:** 9 August 2026, fourth entry — provisional unit-economics evidence register added for the Saudi restaurant interview, Egypt collection standard, KPI definitions and commercial evidence backlog; no price, implementation, blocker or execution-order change<br>
**Last updated:** 10 August 2026 — seven-state application ownership model implemented as a CANDIDATE under `KV-D06-002` and recorded in §0 and §3.3.11, with the new executable queue `docs/KIVO_AGENT_WO_BACKLOG.md`; application-contract change only, no database, migration, route or production change<br>
**Last updated:** 11 August 2026 — `AGENT-WO-002` M-0–M-4 pre-maintenance specification and proof design staged for repository custody under `docs/governance/` as a **candidate in PR #570**; durable custody activates only on governed merge; governance and custody only, no implementation authority<br>
**Last updated:** 12 August 2026 — `KIV-12` / `CTRL-03` **M-0 constraint pre-stage built as a CANDIDATE** on `feat/kiv12-m0-constraint-prestage` under founder-approved `AGENT-WO-002A`, with the authorized migration candidate `0107_kiv12_m0_constraint_prestage.sql`; Builder evidence only, pending independent Quality exact-head review — not accepted, not merged, not applied, no production action<br>
**Last updated:** 13 August 2026 — `KIV-13` / `CTRL-04` **M-1 ADDITIVE SCOPE-1 built as a CANDIDATE** on `feat/kiv13-m1-additive-scope1`, stacked on the Quality-accepted `KIV-12` M-0 head, with the authorized migration candidate `0108_kiv13_m1_additive_scope1.sql`; Builder evidence only, pending independent Quality exact-head review — not accepted, not merged, not production-applied, no production database action<br>
...
**Unit-economics evidence register — 9 August 2026 — PROVISIONAL / UNVERIFIED.** [`docs/KIVO_UNIT_ECONOMICS_EVIDENCE_REGISTER_2026-08-09.md`](./KIVO_UNIT_ECONOMICS_EVIDENCE_REGISTER_2026-08-09.md), SHA-256 `f95def2fa54f161e228963bc719ebe4befb4a528bb43e0ddfba3611fb5a197ad`, 14,688 bytes and 295 lines, preserves a founder-reported Saudi restaurant interview as **USER-REPORTED / UNVERIFIED** evidence: nine call-centre agents at an average SAR 5,500 base salary, about 80 team orders/day and SAR 200 average order value. Under an explicit 30-day arithmetic assumption this is SAR 49,500 base salary/month, 2,400 orders/month, SAR 480,000 gross order value/month and SAR 20.63 base salary/order; the inputs, channel split, operating days, fully loaded labour cost and contribution profit remain unverified. Because Karim V1 addresses WhatsApp rather than telephone calls, only unique eligible WhatsApp order-intent conversations are Kivo-addressable. The register defines Correctly Accepted Order Conversion Rate, correctly accepted orders per operator-attention hour, separate call-abandonment and WhatsApp first-response-SLA metrics, the all-conversation AI/platform cost denominator, Bahaa's same-period multi-source checklist and backlog `COM-EVID-001`, `COM-EVID-002`, `FIN-UNIT-001`, `FIN-BSP-001`, `FIN-MARGIN-001` and `FIN-PRICE-001`. One interview is not a market benchmark; the earlier SAR 1,900 and cost figures remain scenarios, not an approved price or forecast. This documentation authorizes aggregated evidence collection only. It changes no technical blocker or ordering, authorizes no code, model change, contract term, customer contact, SQL, migration, database/production access, deployment, Meta/WhatsApp action, printer mechanism or pilot launch, and makes no employee-replacement or savings claim. Pilot remains **NO-GO**.

**Seven-state application ownership model — 10 August 2026 — CANDIDATE / ACTIVATES ON AUDITED MERGE.** The `KV-D06-002` hard prerequisite is implemented in the change carrying this record: one canonical, dependency-free application contract at `lib/conversation-control/model.ts` now owns the seven-state tuple `AI_ACTIVE, HOLD_UNCLAIMED, HUMAN_ACTIVE, HUMAN_IDLE, AI_RESUME_PENDING, SYSTEM_HOLD, CLOSED`, the authority mapping, the legal-transition table and the claimability rules, and `lib/db/ownership.ts`, `lib/console/conversation-control.ts` and `lib/types.ts` consume it instead of maintaining three independent definitions. Every previous public export name is preserved. The transition graph is unchanged — it is the already-governed superset, so no edge is added or removed and #87 is untouched. `docs/KIVO_AGENT_WO_BACKLOG.md` is added as the operational executable queue; the roadmap remains authoritative for governance and history. This is an **application-contract change only**: no database, migration, migration label, route, RPC, message, order, flag, configuration, deployment, Supabase, Meta, Vercel or production change, and **no one-writer, atomic-claim, RLS, grant or durable-audit claim is made**. **`KV-D06-002` remains OPEN while this pull request is unmerged.** After independent KIVO-AUDITOR clearance of the exact head, the founder's merge of that exact head and verified merge, the application-model obligation closes automatically without another documentation amendment; migration-specification work then becomes eligible only for a new, separately approved work order. Every other blocker, reservation and ordering rule is unchanged — `0105` reserved, `0106` reserved and BLOCKED, `0077` unapplied and unauthorized. Pilot remains **NO-GO**.

**M-0–M-4 pre-maintenance specification custody and queue governance — 11 August 2026 — CANDIDATE WHILE PR #570 IS UNMERGED / ACTIVATES AS AUTHORITATIVE ON GOVERNED MERGE.** **Status.** While PR #570 is unmerged, the two frozen artifacts named below are staged byte-identically in that candidate pull request and are **not yet durable `main` custody**. Authoritative, durable repository custody activates only after **Quality exact-head acceptance → KIVO-AUDITOR exact-head clearance → founder merge → independent merge verification**. After that governed merge verification, the existing `AGENT-WO-002` self-transition rule in `docs/KIVO_AGENT_WO_BACKLOG.md` makes `AGENT-WO-002` **VERIFIED COMPLETE** without another documentation amendment. Every substantive specification and adjudication fact recorded below is unchanged by this staging status. The `AGENT-WO-002` specification and proof design `WO-SPEC-KVD06-M0-M4-ADDITIVE-CONTROL-PLANE-001` has passed the Product & Engineering Quality Gate and independent KIVO-AUDITOR clearance at frozen SHA-256 `61f9d74378f60b025112f6e98455b168067e66b9f2caa545f57dc74c491a4364` (9,819 bytes), and the binding governance adjudication of 11 August 2026 is recorded at SHA-256 `8eb016e5bfd583310bd8627b0eb6108049bdea240ce1c0f8283ef648c5df3a9e` (11,034 bytes). Both are held byte-identically in this repository at [`docs/governance/WO-SPEC-KVD06-M0-M4-ADDITIVE-CONTROL-PLANE-001.CLEARED.md`](./governance/WO-SPEC-KVD06-M0-M4-ADDITIVE-CONTROL-PLANE-001.CLEARED.md) and [`docs/governance/WO-SPEC-KVD06-M0-M4-ADDITIVE-CONTROL-PLANE-001.ADJUDICATION-2026-08-11.md`](./governance/WO-SPEC-KVD06-M0-M4-ADDITIVE-CONTROL-PLANE-001.ADJUDICATION-2026-08-11.md). **Limited L-A supersession.** PR #569 remains the authoritative application claimability contract: `CLAIMABLE_FROM` is exactly `AI_ACTIVE`, `HOLD_UNCLAIMED`, `HUMAN_IDLE`, `SYSTEM_HOLD`; `HUMAN_ACTIVE` is not a claimable source; and **unassigned `HUMAN_ACTIVE` is not claimable**. The adjudication supersedes Revision 14 §7.1 D7, its declared `canClaim` divergence, §13's L-A “PRESERVE and CLAIMABLE” classification and §15.4 R-3's `canClaim` widening **only to that extent**; no other Revision 14 state, transition, safety, tenant, ownership, audit or rollout requirement is superseded, and PR #569 is not reopened, rewritten or bypassed. **R-3 entry gate.** Before R-3, a separately authorized contemporaneous L-A disposition must produce independently accepted evidence proving `count(conversations where ownership_state='HUMAN_ACTIVE' and assigned_member_id IS NULL) = 0`, that the preflight covered the complete governed population, that every identified row has a recorded disposition, and that no member was fabricated or silently assigned. **No hard-coded historical L-A count is an execution invariant** — the historical count of four is source evidence only, never a gate value. **M-2 function identity.** The surviving PostgreSQL identity across M-2 is exactly `public.log_assignment_event() RETURNS trigger`, zero arguments, repaired in place with its OID, trigger `trg_log_assignment_event` and `tgfoid` continuous; `kv_legacy_assignment_bridge` is **behavioral terminology only** and names no PostgreSQL function. **Authority boundary.** `AGENT-WO-002`'s specification and proof design are independently cleared, and that clearance carries **zero implementation authority**: no SQL, no migration drafting, no migration number, no database or Supabase access, no production action, no deployment, and no entry to `P0-MAINT-01` or M-5+. Signed **AD-2** is preserved unchanged — pre-maintenance execution precedes `P0-MAINT-01`, which precedes M-5+. `KV-D06-001` / `P0-CTRL-01` remains **CRITICAL, OPEN and BLOCKING**; `0106` remains reserved and **BLOCKED**; `0077` remains **unapplied and unauthorized**; earlier Revision 14 and preflight records are retained unchanged as historical evidence. Pilot remains **NO-GO**.

**KIV-12 / CTRL-03 M-0 constraint pre-stage — 12 August 2026 — BUILT AS A CANDIDATE / QUALITY REVIEW PENDING.** **Entry gate.** `AGENT-WO-002` reached **VERIFIED COMPLETE** through its governed merge verification and the pre-existing self-transition rule in [`docs/KIVO_AGENT_WO_BACKLOG.md`](./KIVO_AGENT_WO_BACKLOG.md); founder-approved **`AGENT-WO-002A`** implementation authority now exists for its governed bounded implementation-and-proof sequence; and the `KIV-11` / `CTRL-02` **PF5/PF6 entry evidence is independently accepted at the Product & Engineering Quality Gate**. The M-0 entry gate was therefore satisfied before implementation began. **What was built.** `KIV-12` / `CTRL-03` **M-0 CONSTRAINT PRE-STAGE — NOT ADDITIVE** is built as a **candidate** on branch `feat/kiv12-m0-constraint-prestage`, carrying the single authorized migration candidate `supabase/migrations/0107_kiv12_m0_constraint_prestage.sql`. It adds exactly three constraints to `public.conversations`, all VALID: `conversations_restaurant_id_id_key` UNIQUE `(restaurant_id, id)`; `conversations_channel_v1_check` CHECK `channel IN ('whatsapp','website')`; and `conversations_safety_state_check` CHECK `is_safety_hold = false OR ownership_state IN ('SYSTEM_HOLD','HUMAN_ACTIVE','HUMAN_IDLE')`. It creates no M-1 or M-2 object and performs no data remediation. **Evidence status.** The Builder reported a local disposable-PostgreSQL proof of **46 / 46 PASS** against that exact migration artifact. That proof and the implementation are **BUILDER EVIDENCE ONLY** until independent Quality review accepts the final exact head. **M-0 is not accepted, not merged, not applied, not production-deployed and not production proven.** **Production application of `0107` has not occurred.** During M-0 implementation, production access was **ZERO**: zero production queries, zero production writes, zero production DDL and zero production remediation. **Deployment truth.** Automatic Vercel Preview deployment(s) occurred for PR #571. These are non-production preview deployments and did not constitute production deployment or production database application of `0107`. **Production deployments = ZERO.** **Primary proof evidence, custodied for independent Quality verification.** The executable proof lives at [`scripts/proof-kiv12-m0-disposable-pg.py`](../scripts/proof-kiv12-m0-disposable-pg.py) and its complete raw execution transcript at [`docs/governance/KIV-12_M0_DISPOSABLE_PG_PROOF_2026-08-12.txt`](./governance/KIV-12_M0_DISPOSABLE_PG_PROOF_2026-08-12.txt). Both are supplied so Quality can inspect and re-run the proof rather than trust a Builder narrative. **Downstream.** `M-1` / `KIV-13` remains downstream and must not begin until the M-0 gate is independently accepted and KIVO-PM processes that verdict. `R-3` remains **HARD STOP**. `0105` remains reserved; `0106` remains reserved and **BLOCKED**; `0077` remains **unapplied and unauthorized**. Pilot remains **NO-GO**.

**KIV-13 / CTRL-04 M-1 additive SCOPE-1 — 13 August 2026 — BUILT AS A CANDIDATE / QUALITY REVIEW PENDING.** **Entry gate.** `KIV-12` / `CTRL-03` M-0 was **independently accepted at the Product & Engineering Quality Gate at exact head `f20b56889dc354a411b0b46cd627b355fc732183`**; its pull request **#571 remains draft and unmerged**, and migration `0107_kiv12_m0_constraint_prestage.sql` is **not production-applied**. M-1 is built as a **stacked candidate** on that exact accepted head, not on `main`. **Binding sources.** Revision 14 was supplied and independently verified at SHA-256 `f5eda59d06bdb4e72af183ab70deaec5dbb0041a02b6c5ba59bad21e456f6c37`, 156,719 bytes, 2,140 lines, and reconciled with the CLEARED specification, the 11 August adjudication and the founder-approved **KIV-13 M1 Binding Adjudication of 13 August 2026**, which resolves the A0/A2 evidence-trigger stage assignment and the seven preserved owner-policy identifiers. **SA1 is implemented as the actor authority; SA2 remains WITHDRAWN and is not implemented** — consistent with Revision 14 §8.3 SA6, which forbids issuing a migration containing SA2 before VP-1 has been executed. **What was built.** The single authorized migration candidate `supabase/migrations/0108_kiv13_m1_additive_scope1.sql` creates the role `kivo_control_owner` (granted to no role), its additive schema/table/column privileges, the seven preserved owner policies, `member_identity_versions` with `ux_miv_open_version`, its three immutability triggers and four policies, the **population-relative** MIV initialization, A0 `public.control_operations` with `ux_a0_evidence_core` and its four evidence triggers, A2 `public.conversation_audit_failures` with `fk_a2_parent_core` and its four evidence triggers, the **eight nullable** A1 columns, the type `public.kv_control_result`, and the sixteen SCOPE-1 control functions `F1..F16` with their SA1 EXECUTE separation. **It is additive**: it revokes no privilege, changes no existing owner, enables or forces RLS on no existing table, adds no trigger to an existing table, adds no constraint to a populated table and writes no existing row. **Evidence status.** A local disposable-PostgreSQL proof reported **119 / 119 PASS** against that exact artifact, including an end-to-end governed operation producing one A0 row and one canonical A1 row, D5 replay, D6 no-op and the A2 evidence path. That proof and the implementation are **BUILDER EVIDENCE ONLY** until independent Quality review accepts the final exact head. **M-1 is not accepted, not merged, not production-applied and not production proven.** Production database actions were **ZERO** — zero production queries, writes, DDL, migration application and remediation, and zero production deployments. Automatic Vercel Preview deployment(s) may occur for the M-1 pull request; those are **non-production preview deployments** and constitute neither production deployment nor production database application of `0108`. **Downstream.** `M-2` has **not started** and remains the sole owner of the audit-bridge repair and of `tg_members_identity_version`; the four A1 evidence triggers and all A1 hardening remain **M-5**. `R-3` remains **HARD STOP**. `0105` remains reserved; `0106` remains reserved and **BLOCKED**; `0077` remains **unapplied and unauthorized**. Pilot remains **NO-GO**.

**KIV-13 / CTRL-04 M-1 — QUALITY REJECTION CORRECTION — 13 August 2026 — CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING.** This record **supersedes the preceding 13 August M-1 record**, which described the candidate at the now-rejected head `3dddc7267ba31d34aff9361c0acb4b0517b35727`; that head is preserved in branch history as evidence and is not rewritten. Quality returned **QUALITY GATE REJECTED — KIV-13 M1 CORRECTION REQUIRED** on five behavioural defects, all of which are corrected: **(A) claimability** — the 11 August binding adjudication supersedes Revision 14 §7.1 D7, so `CLAIMABLE_FROM` is exactly `AI_ACTIVE`, `HOLD_UNCLAIMED`, `HUMAN_IDLE`, `SYSTEM_HOLD`; `HUMAN_ACTIVE` succeeds only as an idempotent D6 when already assigned to the claiming member, unassigned `HUMAN_ACTIVE` raises KIV14 and another member's assignment raises KIV15, with no legacy exception and no bypass of PR #569; **(B) actor authority** — F5 and F9 now require the current assignee or a manager, and F8 requires the assignee when human-owned and a manager when unassigned, AI-owned or system-owned; **(C) SB4** — every applied transition writes the governed presentation status in the same UPDATE, while no-op and alerted outcomes write nothing and a stale status can neither block a transition nor manufacture a no-op; **(D) F14 L2 order** — the A0 replay now precedes the row re-read and create-or-adopt, so a replay returns the recorded result without creating, adopting or mutating; **(E) generic KIV11** — all ten non-creation operations resolve tenant and conversation with one combined predicate and one identical generic error, so the API is not an existence oracle. The proof harness grew from 119 to **190 assertions**, none weakened, with the previously weak function-inventory check replaced by an exact sixteen-entry name-and-argument-type comparison. The corrected candidate reports **190 / 190 PASS** from a clean committed tree. **A1 EVENT_TYPE 8-vs-13 — PRE-R3 GOVERNANCE / STAGE-ORDERING BLOCKER — FOUNDER/PM SPEC ADJUDICATION REQUIRED.** The live `0099` `conversation_assignment_events.event_type` CHECK admits **eight** values; Revision 14 A1-2 requires **thirteen**. The five missing — `HOLD_RELEASED`, `REOPENED`, `IDLE`, `TIMEOUT_RETURNED`, `STALE_ASSIGNEE_CLEARED` — are emitted by five governed operations, so those operations cannot complete an applied transition until A1-2 exists, yet A1-2 belongs to **M-5** while callers convert at **R-3**. This is **not** authority to widen the populated CHECK: `0108` does not widen it, and the proof asserts that it does not. Those five operations are proven correct in a clearly labelled supplementary fixture carrying A1-2. **Current truth.** `AGENT-WO-002` **VERIFIED COMPLETE**; `AGENT-WO-002A` **IN PROGRESS**; M-0 an independently Quality-accepted candidate at `f20b56889dc354a411b0b46cd627b355fc732183`, still unmerged and not production-applied; M-1 a **CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING**, not accepted, not merged, not production-applied; **M-2 NOT STARTED**; `R-3` **HARD STOP**; `0105` reserved, `0106` reserved and **BLOCKED**, `0077` unapplied and unauthorized; pilot **NO-GO**. Production database actions remain **ZERO**; automatic Vercel Preview deployment(s) for the pull request are **non-production**.

**KIV-13 / CTRL-04 M-1 — FURTHER QUALITY CORRECTION — 13 August 2026 — FURTHER-CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING.** Supersedes the two preceding 13 August M-1 records, which described the earlier candidates at heads `3dddc7267ba31d34aff9361c0acb4b0517b35727` and `2b015027b06db24af9d05c29b3d5f566ab3e777c`; both are preserved as ancestors and are not rewritten. Quality found five further defects, all corrected: **(1)** operation-specific actor authority for F5, F8 and F9 now runs at **D2**, before D4, D5 and D6, so an unauthorized member no longer receives a successful no-op merely because the conversation already sits at the operation target; **(2)** F14 now makes the actual fingerprint replay decision after both locks and **before** any conversation re-read, create or adopt — a reused `operation_id` with a changed customer or channel returns **KIV19** from the fingerprint rather than KIV26 from adoption, which is the discriminating evidence; **(3)** the create_safety_conversation fingerprint now carries `p_customer_id`, `p_channel`, `p_escalation_reason`, `p_allergy_note`, `p_reason` in the exact §16.2 order, the first two having been omitted; **(4)** F13 now resolves replay before reading the conversation or tenant configuration and fingerprints all five expected timeout inputs in the governed order, so changing any one alone yields a deterministic KIV19 and a valid replay survives a later configuration change; **(5)** governed fingerprint text is **NFC-normalized** with the core `normalize(value, NFC)` built-in — no extension, no new object and no sixth path — so canonically equivalent composed and decomposed text share a fingerprint while a materially different value still raises KIV19. The proof harness grew 190 → **235 assertions**, none weakened, and reports **235 / 235 PASS** from a clean committed tree. No previously accepted correction is regressed: PR #569 claimability semantics, HUMAN_ACTIVE same-assignee idempotence, SB4 write-through, the silent no-op, the combined generic KIV11, the exact F1..F16 inventory, the M-1 additive law, population-relative MIV, and policy/grant/SA1 behaviour are all re-proven. **`KIV-74` is unchanged and separate:** the A1 `event_type` 8-vs-13 gap remains an **OPEN PRE-R3 GOVERNANCE / STAGE-ORDERING BLOCKER requiring FOUNDER/PM SPEC ADJUDICATION**, and `0108` still does not widen the populated CHECK. **Current truth.** `AGENT-WO-002` **VERIFIED COMPLETE**; `AGENT-WO-002A` **IN PROGRESS**; M-0 an independently Quality-accepted candidate at `f20b5688…`, unmerged and not production-applied; M-1 a **FURTHER-CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING**; **M-2 NOT STARTED**; `R-3` **HARD STOP**; pilot **NO-GO**. Production database actions **ZERO**; automatic Vercel Preview deployment(s) **OCCURRED** and are **non-production**.

**KIV-13 / CTRL-04 M-1 — CONCURRENCY AND REQUEST-IDENTITY CORRECTION — 13 August 2026 — CONCURRENCY-CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING.** Supersedes the three preceding 13 August M-1 records, which described the candidates at heads `3dddc7267ba31d34aff9361c0acb4b0517b35727`, `2b015027b06db24af9d05c29b3d5f566ab3e777c` and `a1f173feb2a3ec336f22c9bea902a5817974a410`; all three are preserved as ancestors of the corrected head and none is rewritten. Quality found two behavioural defects, both corrected on a normal child commit. **(1) F13 lock ordering.** `kv_sys_control_timeout_return` now acquires the governed conversation **row lock (T6) BEFORE** the T7 expected-state and configuration validation, using `SELECT ... FOR UPDATE`. Every expectation check and the transition decision are evaluated against the locked row and the lock is held for the remainder of the transaction, so the universal core's own re-read observes exactly that row; a stale timeout can no longer be validated against a row a concurrent governed operation then moves, and therefore can never overwrite a fresh human claim or reassignment. **(2) Authoritative request identity.** The two operations whose §16.2 fingerprint fields are not all present in the universal core's governed signature — `create_safety_conversation` (customer, channel) and `timeout_return` (the five expected inputs) — previously had those fields **reconstructed inside the core from mutable conversation and tenant state**, so an exact retry could hash differently and be spuriously rejected KIV19. F13 and F14 now compute the canonical §16.2 identity from the **original caller inputs** and hand it to the core transaction-locally; the core consumes it once, clears it so it cannot leak to a later operation in the same transaction, and refuses those two operations outright if it is absent or malformed. **No governed public signature changed, no new object, extension or authority was introduced and no sixth path was added.** **Proof.** The harness grew 235 → **251 assertions**, none weakened, and adds deterministic concurrency tests that exercise **actual PostgreSQL lock and serialization behaviour rather than simulating it with sequential row edits**: a genuine second session holds the conversation row lock while F13 runs under `statement_timeout` and F13 demonstrably blocks and writes no A0 row; a fresh governed human claim commits before a stale timeout arrives, which is then refused KIV21 with ownership and assignee intact and no A0 row written; and both F13's and F14's stored A0 fingerprints are compared against an **independent SQL §16.2 oracle**, proven invariant across conversation and configuration drift and still replayable on an exact retry. Migration `0108` SHA-256 `8a435974b1b302223548b4aeadfa84ee0a11918a301d8698f7774a49f757a107` (80,100 bytes). The candidate reports **251 / 251 PASS** from the clean committed head `66e4e20a49fa11e0c80e592aa51c637337290682`; the complete raw transcript is custodied byte-for-byte at [`docs/governance/KIV-13_M1_DISPOSABLE_PG_PROOF_2026-08-13.txt`](./governance/KIV-13_M1_DISPOSABLE_PG_PROOF_2026-08-13.txt) and the executable proof at [`scripts/proof-kiv13-m1-disposable-pg.py`](../scripts/proof-kiv13-m1-disposable-pg.py), so Quality can re-run the proof rather than trust a Builder narrative. **No previously accepted correction is regressed** — PR #569 claimability semantics, HUMAN_ACTIVE same-assignee idempotence, D2 actor authority for F5/F8/F9, SB4 write-through, the silent no-op, the combined generic KIV11, F14's replay-before-adopt ordering, NFC fingerprint normalization, the exact F1..F16 inventory, the M-1 additive law, population-relative MIV and policy/grant/SA1 behaviour are all re-proven. **`KIV-74` is unchanged and separate:** the A1 `event_type` 8-vs-13 gap remains an **OPEN PRE-R3 GOVERNANCE / STAGE-ORDERING BLOCKER requiring FOUNDER/PM SPEC ADJUDICATION**, and the proof asserts that `0108` still does not widen the populated CHECK. **Current truth.** `AGENT-WO-002` **VERIFIED COMPLETE**; `AGENT-WO-002A` **IN PROGRESS**; M-0 an independently Quality-accepted candidate at `f20b5688…`, unmerged and not production-applied; M-1 a **CONCURRENCY-CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING**, not accepted, not merged, not production-applied and not production proven; **M-2 NOT STARTED**; `R-3` **HARD STOP**; `0105` reserved, `0106` reserved and **BLOCKED**, `0077` unapplied and unauthorized; pilot **NO-GO**. Production database actions remain **ZERO** — zero production queries, writes, DDL, migration application and remediation, and zero production deployments; automatic Vercel Preview deployment(s) for the pull request **OCCURRED** and are **non-production**.

**KIV-14 / KIV-77 — 0108 TRANSACTIONAL-BOOTSTRAP SOURCE CORRECTION — 14 August 2026 — CORRECTED CANDIDATE / INDEPENDENT REVIEW REQUIRED.** **Production truth first.** `0107` / M-0 is **applied and read back in production** (`MaitreAi` / `zlighrbsjexrozrmuwpw`), and `0108` / M-1 is **not applied**: the production attempt failed closed and rolled back completely, leaving `public.members` unchanged at N=20. **Verified defect.** Supabase executes migrations as `postgres` (`rolsuper=false`, `rolcreaterole=true`). PostgreSQL 16+ records the automatic `CREATE ROLE` grant as **ADMIN TRUE / SET FALSE / INHERIT FALSE**, so the executor cannot `SET ROLE` the new owner and the first ownership transfer aborts with SQLSTATE **42501 `must be able to SET ROLE "kivo_control_owner"`**; granting the new owner schema `CREATE` alone does not resolve it. **Correction, entirely within `0108`.** A new section 1B grants the minimum temporary bootstrap capability — `kivo_control_owner` to the executing role `WITH INHERIT TRUE, SET TRUE`, plus `CREATE` on schema `public` to `kivo_control_owner`. `SET` is required for the ownership transfers; `INHERIT` is required because later statements act on objects `kivo_control_owner` already owns — the MIV initialization INSERT under FORCE RLS, the section 5 and 15 grants, and the A0/A2 foreign keys onto MIV. A new section 16 removes both and then **verifies the removal, failing closed**, so incomplete cleanup aborts and rolls the whole migration back. **Final state is the accepted M-1 contract, unchanged**: `kivo_control_owner` still owns MIV, A0, A2, `kv_control_result` and the governed functions; role attributes, grants, policies, RLS and EXECUTE exposure are untouched; no existing member row is written. Per the **14 August adjudication**, exactly one membership may survive — the record PostgreSQL creates automatically and the executor cannot revoke, at **ADMIN TRUE / SET FALSE / INHERIT FALSE** with the bootstrap superuser as grantor — and after cleanup the executor provably cannot `SET ROLE kivo_control_owner` and inherits none of its object permissions. **Proof.** Local disposable **PostgreSQL 17.11**, migrations applied by a non-superuser `CREATEROLE` executor in one transaction: **288 / 288 PASS** from clean committed head `2738a70b0dcb894317b5a7cb8bb0f166fd2e9690`, including the historical `0108` failing for the exact production reason, complete rollback under failure injected after bootstrap creation and after the first ownership transfer, and full bootstrap-cleanup verification. The transcript is custodied at [`docs/governance/KIV-14_0108_BOOTSTRAP_CORRECTION_PROOF_2026-08-14.txt`](./governance/KIV-14_0108_BOOTSTRAP_CORRECTION_PROOF_2026-08-14.txt). The historical accepted `0108` (`8a435974…f757a107`) remains immutable at `bf5f982b…` as baseline evidence; the corrected candidate is `7b24f4ea7f8c6ae56d6469cc998a71fcfb5cdd19f942c2701709a2f796701b92`. **Assertion history, stated honestly.** The historical combined **zero-membership** assertion **was replaced** — it is not accurate to claim that no assertion was removed, skipped or weakened. Inbound membership changed from **zero to at most one**, which is a **relaxation**; outbound membership remains **zero**; the one permitted inbound row must be the **exact unusable automatic row**; and the added shape, bootstrap-superuser grantor, no-`SET`, no-`INHERIT`, `SET ROLE`-denied and no-inherited-permission checks **compensate for that approved relaxation**. **Boundaries.** No production access occurred in this work; no production retry of `0108` is authorized; not merged. `M-2`/`KIV-15` not started; `KIV-74` open; `R-3` **HARD STOP**; `0105` reserved; `0106` reserved and **BLOCKED**; `0077` unapplied and unauthorized; pilot **NO-GO**.

**KIV-77 — 0108 BOOTSTRAP CORRECTION — CYCLE 2 (KIV-81 FINDINGS W1–W4) — 15 August 2026 — CORRECTED CANDIDATE / INDEPENDENT RE-REVIEW REQUIRED.** Supersedes the 14 August KIV-77 record for the candidate state only; that record's dated content is retained unchanged apart from the W3 wording correction it required. **W1 — grantor verification.** `0108`'s section 16 safety check now also verifies the **grantor**: any membership on `kivo_control_owner` recorded by a **non-superuser** grantor aborts the migration, because the one permitted record is the automatic `CREATE ROLE` grant whose grantor is the bootstrap superuser; a membership from an ordinary grantor is one the executor could have removed, so its survival means cleanup did not complete. **W2 — PUBLIC holds schema `CREATE`.** A new disposable PostgreSQL 17 test covers the case where `PUBLIC` already holds `CREATE` on schema `public`. The bootstrap-only grant then cannot be cleanly withdrawn, because `has_schema_privilege()` still reports `CREATE` via `PUBLIC`; the migration **aborts on the governed fail-closed check and rolls back completely** — no role, no M-1 object, no A1 column. **W3 — accurate characterisation.** Every claim that the membership assertion is “strictly stronger” is **removed**. The accurate statement is recorded instead: under the approved 14 August adjudication the permitted **inbound membership count changed from zero to at most one — a relaxation, not a tightening** — while `SET` and `INHERIT` remain forbidden on any surviving membership and outbound membership remains forbidden entirely. **W4 — failure-safe, task-scoped teardown.** Every artifact is namespaced by a unique run tag; the run directory is `0700` owned by `postgres`; temp and injected SQL files are `0600` owner-only rather than world-readable in shared `/tmp`; the port is allocated dynamically and the unix socket lives inside the run directory. Teardown stops the cluster, kills only processes carrying this run's tag, and removes PGDATA, the socket and every temp/injected file; it never raises, is idempotent, and is registered with `atexit` **before any cluster exists**, so it still runs when `initdb`/`pg_ctl` never succeed — verified out of band with an unusable `PGBIN`, which left nothing behind. **Proof.** Disposable **PostgreSQL 17.11**, applied by the non-superuser executor in one transaction: **302 / 302 PASS** (288 → 302) from clean committed head `9850a88b4fe6a01452d28b02ac68e7c28e4abdab`; transcript custodied at [`docs/governance/KIV-14_0108_BOOTSTRAP_CORRECTION_PROOF_2026-08-14.txt`](./governance/KIV-14_0108_BOOTSTRAP_CORRECTION_PROOF_2026-08-14.txt). Corrected `0108` is now `d0e9a595399dcf60e653c92b05fd4d5174db3a6488200f8d0ab050fc1f9fb50b`; the historical accepted `8a435974…f757a107` remains immutable at `bf5f982b…`. **Recorded limitation.** A state with exactly one surviving membership whose grantor is a non-superuser cannot be constructed — granting a role requires the grantor to hold a membership row — so the W1 grantor guard is defence-in-depth exercised alongside the count guard. **Boundaries.** No production access; no production `0108` retry; not merged; PR #573 remains **draft**. `M-2`/`KIV-15` not started; `KIV-74` open; `R-3` **HARD STOP**; `0105` reserved; `0106` reserved and **BLOCKED**; `0077` unapplied and unauthorized; pilot **NO-GO**.

**KIV-87 — PR #573 QUALITY CORRECTION (KIV-85 FINDINGS Q1–Q5) — 15 August 2026 — CORRECTED CANDIDATE / INDEPENDENT REVIEW REQUIRED.** Independent Quality returned `QUALITY GATE HARD STOP — EVIDENCE OR ENVIRONMENT GAP` on head `c9acf467…`: custody and static review passed, but the reviewer's environment had PostgreSQL 16 only, so the governed proof could not be independently rerun. Five corrections are made. **Q1 — the file-mode claim is now true.** `historical_0108.sql` was written `0644` while the evidence claimed every task-created SQL artifact was `0600` owner-only; it now uses the same owner-only helper, and an executable check verifies **every** `.sql` file in the run directory is `0600` and owned by `postgres`. No confidentiality exposure existed — the run directory is `0700` and the file holds the already-public historical migration — but the blanket claim was false. **Q2 — assertion history is now honest.** **Assertion history, stated honestly.** The historical combined **zero-membership** assertion **was replaced** — it is not accurate to claim that no assertion was removed, skipped or weakened. Inbound membership changed from **zero to at most one**, which is a **relaxation**; outbound membership remains **zero**; the one permitted inbound row must be the **exact unusable automatic row**; and the added shape, bootstrap-superuser grantor, no-`SET`, no-`INHERIT`, `SET ROLE`-denied and no-inherited-permission checks **compensate for that approved relaxation**. **Q3 — the whole-file transaction wrapper is a mandatory, fail-closed execution precondition**, recorded in the `0108` header and proven executably: the *same* file with the *same* injected failure leaves the role and a usable `SET`/`INHERIT` grant behind when applied statement-by-statement, and leaves nothing when applied with `psql --single-transaction`. `0108` deliberately carries no top-level `BEGIN`/`COMMIT`, so it **must never be applied statement by statement**. **Q4 — a separately Founder-authorized SELECT-only production preflight** for `has_schema_privilege('public','public','CREATE')` is recorded in the `0108` header as mandatory before any future production application; if `PUBLIC` holds schema `CREATE` the migration is designed to abort and roll back, and no permission repair may be improvised. The executable PostgreSQL 17 proof of that abort and complete rollback is retained. **Q5 — limitations remain explicit**: the hostile W1 grantor state cannot be isolated behaviourally because the membership-count guard fires first; the accompanying source-substring assertion is **not** full independent behavioural proof; and `INHERIT` is required by **this candidate's current statement order**, not claimed unavoidable under every possible design. **Proof.** Disposable **PostgreSQL 17.11**, non-superuser executor, whole-file transaction: **315 / 315 PASS** from clean committed head `d772ff449418076799ca78a388231c33f4a7f9b1`; corrected `0108` is `8b3aa745f8e60178ebda7a9101fc4d654e252b8d27372924434d00345d59090c`. **Boundaries.** No production access; no production `0108` retry; no permission repair; not merged; PR #573 remains **draft**. `M-2`/`KIV-15` not started; `KIV-74` open; `R-3` **HARD STOP**; `0105` reserved; `0106` reserved and **BLOCKED**; `0077` unapplied and unauthorized; pilot **NO-GO**.

**KIV-126 — 0108 MEMBER-IDENTITY AND PRE-EXISTING ROLE GUARD (KIV-124 P1) — 16 August 2026 — CORRECTED CANDIDATE / INDEPENDENT ENGINEERING REVIEW REQUIRED.** Closes the two `KIV-124` P1 source gaps, both inside `0108` and both fail-closed under the mandatory whole-file transaction wrapper. **(1) Executor identity.** Section 16 verified a surviving membership's count, shape and grantor but not **who held it**. The one record the adjudication permits is the automatic `CREATE ROLE` grant made to the executing role itself; a membership held by any other role is a standing grant the migration neither made nor can account for, leaving a third party able to administer `kivo_control_owner` after install. Section 16 now rejects any membership whose member is not `current_user`, ordered **before** the count guard so the failure names the offender. **(2) Pre-existing role attributes.** Role creation is guarded by `IF NOT EXISTS`, so a role of that name created earlier by another actor was adopted silently with whatever attributes it had. A new section-1 check verifies the governed safe set unconditionally — NOLOGIN, NOSUPERUSER, NOBYPASSRLS, NOCREATEDB, NOCREATEROLE, NOINHERIT, NOREPLICATION — and names every unsafe attribute found. **Proof.** Disposable **PostgreSQL 17.11**, faithful non-superuser `CREATEROLE` executor, whole-file transaction: **357 / 357 PASS** from clean committed head `04f73aec3d05091764295bce44acf0f395afd871`, covering an unrelated member aborting the run by name with no M-1 object, A1 column or bootstrap `SET`/`INHERIT` grant surviving; **each of the seven** unsafe attributes aborting with complete rollback; a **safe** pre-existing role being accepted with its governed attributes intact and the executor as sole member; and a second application neither reinitializing nor rewriting MIV. Corrected `0108` SHA-256 `00cd7b7fe2ee581df7b9d038301123db45a80962fe8a6ad3c0435e2893dea9ee`. **All 315 previously accepted checks are preserved and none skipped.** One assertion label changed and is disclosed: the W1 hostile-membership test required the `KIV77` label, but the new KIV-126 member guard fires earlier; the condition is not weakened — it still requires a governed fail-closed guard to abort and now additionally requires the offending member to be named. Membership-count, grantor, ADMIN/SET/INHERIT, schema-`CREATE` cleanup, ownership, MIV, idempotency and transaction-wrapper guarantees are unchanged. **Boundaries.** No production or Supabase access; no production `0108` retry; not merged; draft PR only. `M-2`/`KIV-15` not started; `KIV-74` open; `R-3` **HARD STOP**; `0105` reserved; `0106` reserved and **BLOCKED**; `0077` unapplied and unauthorized; production remains **M-0 applied / M-1 not applied**, `public.members` **N = 20**; pilot **NO-GO**.
...
**Implementation candidate — 10 August 2026.** The seven-state application ownership work named immediately above is now IMPLEMENTED AS A CANDIDATE under `WO-ENG-KVD06-002-SEVEN-STATE-APPLICATION-MODEL-001`, in the pull request carrying this note. All five listed items are covered: `HOLD_UNCLAIMED` and `AI_RESUME_PENDING` are in the canonical application state set; `LEGAL_TRANSITIONS` is the single shared table both consuming modules read; `setOwnershipState` accepts both new states through that shared table; and escalation reaching `HOLD_UNCLAIMED` is proven at the application layer. Proof is automated and executed from the final committed tree — the focused ownership suite, a new cross-module parity proof, and the existing control-plane proof left unchanged. **`KV-D06-002` remains OPEN while that pull request is unmerged**; on independent clearance of the exact head, founder merge of that exact head and verified merge, the application-model obligation closes automatically with no further documentation amendment. **Migration-specification drafting is not unblocked by this note** — it becomes eligible only through a new, separately approved work order, and no migration label, including `0106`, is assigned or unblocked here. The proof is an application-contract proof only: it establishes nothing about the database, atomic claims, one-writer enforcement, RLS, grants, durable audit or production behaviour. Pilot remains **NO-GO**.
...
**Parallel commercial evidence lane — added 9 Aug 2026.** Aggregated, read-only merchant evidence collection under `COM-EVID-001`, `COM-EVID-002`, `FIN-BSP-001` and `FIN-MARGIN-001` may proceed while the technical sequence continues. This lane is not a predecessor to `KV-D06-002`, does not delay or bypass the seven-state ownership work, does not authorize `FIN-UNIT-001` production measurement before its own safe evidence path exists, and does not approve pricing, implementation, database access or pilot launch. Results must be recorded in `docs/KIVO_UNIT_ECONOMICS_EVIDENCE_REGISTER_2026-08-09.md`; discrepancies remain visible rather than being resolved by preference.
...
| 9 Aug 2026 | Provisional unit-economics evidence register added for the user-reported Saudi interview and Bahaa's Egypt evidence collection. The reported nine agents, SAR 5,500 average base salary, about 80 daily team orders and SAR 200 average order value remain USER-REPORTED / UNVERIFIED; the derived 30-day figures are arithmetic assumptions, not accepted performance. Added CAOCR, operator-attention efficiency, separate call and WhatsApp missed-demand metrics, all-conversation cost-per-correctly-accepted-order rules and `COM-EVID-001`/`COM-EVID-002`/`FIN-UNIT-001`/`FIN-BSP-001`/`FIN-MARGIN-001`/`FIN-PRICE-001`. One interview is not a benchmark; no price, savings, employee replacement, code, model change, contract, SQL, migration, production access, deployment, Meta/WhatsApp action or pilot launch is approved. The seven-state ownership work remains next, all blockers and reservations remain unchanged, and pilot remains NO-GO |
| 10 Aug 2026 | Seven-state application ownership model implemented as a CANDIDATE under `WO-ENG-KVD06-002-SEVEN-STATE-APPLICATION-MODEL-001`, satisfying the `KV-D06-002` hard prerequisite at the application layer. One canonical dependency-free contract, `lib/conversation-control/model.ts`, now owns the seven-state tuple `AI_ACTIVE, HOLD_UNCLAIMED, HUMAN_ACTIVE, HUMAN_IDLE, AI_RESUME_PENDING, SYSTEM_HOLD, CLOSED`, the authority mapping (`AI_ACTIVE`→AI; `HUMAN_ACTIVE`/`HUMAN_IDLE`→HUMAN; the rest→NONE, never dual), the legal-transition table and the claimable source set `AI_ACTIVE, HOLD_UNCLAIMED, HUMAN_IDLE, SYSTEM_HOLD`; `lib/db/ownership.ts`, `lib/console/conversation-control.ts` and `lib/types.ts` consume it, so no second independently maintained state, authority or transition definition remains in those three files. Every previous public export name is preserved, including `OwnershipState`, `OWNERSHIP_STATES`, `isLegalTransition`, `assertLegalTransition`, `CLAIMABLE_FROM`, `canClaim`, `setOwnershipState`, `ControlMode`, `CONTROL_MODES`, `authorityFor`, `canTransition`, `assertTransition` and the RPC wrappers, and both thrown-message prefixes are unchanged. The transition graph is the already-governed superset — no edge added or removed, the direct human-to-AI edges retained, #87 intact, and atomic server-confirmed handback still separate later control-plane work. `docs/KIVO_AGENT_WO_BACKLOG.md` was added as the operational executable queue with its governance rules, twelve ordered rows and the `AGENT-WO-001` self-transition rule; the roadmap remains authoritative for governance and history. Proof is automated and produced from the final committed tree: the expanded focused ownership suite, the new cross-module parity proof `scripts/proof-seven-state-ownership.test.ts`, and `scripts/proof-control.test.ts` left unchanged with no assertion removed, weakened, skipped or rewritten. **This is an application-contract change only.** No SQL, migration, migration specification, migration label, database or Supabase access, route, RPC, message, order, printer, flag, configuration, deployment, Meta, Vercel or production action occurred or is authorized, and no one-writer, atomic-claim, tenant-isolation, RLS, FORCE RLS, grant or durable-audit claim is made. **`KV-D06-002` remains OPEN while the pull request is unmerged**; upon independent clearance of the exact head, founder merge of that exact head and verified merge, the application-model obligation closes automatically without another documentation amendment, and migration-specification work becomes eligible only for a new, separately approved work order. `KV-D06-001` / `P0-CTRL-01`, `P0-MAINT-01`, `P0-ORD-01`, `P0-WA-01`, `P0-SHADOW-01`, `P0-VOICE-01`, B-2, B-4, B-13, the Meta/legal backlog, the narrowed Egypt V1 scope and every other blocker are unchanged; `0105` remains reserved, `0106` remains reserved and BLOCKED, `0077` remains unapplied and unauthorized; and pilot remains NO-GO |
| 11 Aug 2026 | `AGENT-WO-002` M-0–M-4 pre-maintenance specification and proof design staged for durable repository custody under `WO-GOV-KVD06-M0M4-SPEC-CUSTODY-QUEUE-001` as a candidate in PR #570. The specification `WO-SPEC-KVD06-M0-M4-ADDITIVE-CONTROL-PLANE-001` passed the Product & Engineering Quality Gate and independent KIVO-AUDITOR clearance at frozen SHA-256 `61f9d74378f60b025112f6e98455b168067e66b9f2caa545f57dc74c491a4364`, 9819 bytes, and the binding governance adjudication of 11 Aug 2026 is recorded at SHA-256 `8eb016e5bfd583310bd8627b0eb6108049bdea240ce1c0f8283ef648c5df3a9e`, 11034 bytes; both are staged byte-identically under `docs/governance/` in candidate PR #570 and, while PR #570 is unmerged, are **not yet durable `main` repository custody**; durable and authoritative repository custody activates only after **Quality exact-head acceptance → KIVO-AUDITOR exact-head clearance → founder merge → independent merge verification**, after which the existing `AGENT-WO-002` self-transition rule in `docs/KIVO_AGENT_WO_BACKLOG.md` makes `AGENT-WO-002` VERIFIED COMPLETE without another documentation amendment. The adjudication applies a **limited L-A supersession**: PR #569 remains the application claimability authority with `CLAIMABLE_FROM` exactly `AI_ACTIVE`, `HOLD_UNCLAIMED`, `HUMAN_IDLE`, `SYSTEM_HOLD`, and **unassigned `HUMAN_ACTIVE` is not claimable**; only Revision 14 §7.1 D7, its `canClaim` divergence, §13's L-A “PRESERVE and CLAIMABLE” wording and §15.4 R-3's `canClaim` widening are superseded, and nothing else. Before R-3 a separately authorized contemporaneous L-A disposition must prove `count(conversations where ownership_state='HUMAN_ACTIVE' and assigned_member_id IS NULL) = 0` with complete-population coverage, a recorded per-row disposition and no fabricated or silently assigned member; **no hard-coded historical L-A count is an execution invariant**. M-2's surviving function identity is exactly `public.log_assignment_event() RETURNS trigger` with zero arguments, repaired in place with unchanged OID and a continuous `trg_log_assignment_event` trigger; `kv_legacy_assignment_bridge` is behavioral terminology only and no `public.kv_legacy_assignment_bridge()` function exists or may be created. `AGENT-WO-002`'s specification and proof design are independently cleared, which grants **zero implementation authority**. Backlog governance updated in the same change: `AGENT-WO-002` moves to AUDIT CLEARED / MERGE PENDING, new `AGENT-WO-002A` records the BLOCKED pre-maintenance implementation increment M-0 → M-1 → M-2 → R-3 → R-4 with R-3 additionally blocked until the adjudicated L-A evidence gate is independently accepted and `P0-VOICE-01` is complete, and `AGENT-WO-003` / `P0-MAINT-01` now depends on verified completion of `AGENT-WO-002A`. Documentation and custody only: no code, SQL, migration, migration number, database or Supabase access, configuration, deployment, Meta, Vercel or production action occurred or is authorized. Signed AD-2 is preserved — pre-maintenance execution precedes `P0-MAINT-01`, which precedes M-5+. `KV-D06-001` / `P0-CTRL-01` remains CRITICAL, OPEN and BLOCKING; `0105` remains reserved; `0106` remains reserved and BLOCKED; `0077` remains unapplied and unauthorized; all earlier Revision 14 and preflight records are retained unchanged as historical evidence; and pilot remains NO-GO |
| 12 Aug 2026 | `KIV-12` / `CTRL-03` **M-0 CONSTRAINT PRE-STAGE — NOT ADDITIVE** built as a **candidate** on `feat/kiv12-m0-constraint-prestage` under founder-approved `AGENT-WO-002A`. Entry gate satisfied first: `AGENT-WO-002` is **VERIFIED COMPLETE** through governed merge verification and its pre-existing self-transition rule, founder `AGENT-WO-002A` implementation authority exists, and the `KIV-11` / `CTRL-02` PF5/PF6 entry evidence is independently accepted at the Product & Engineering Quality Gate. The change carries exactly one authorized migration candidate, `supabase/migrations/0107_kiv12_m0_constraint_prestage.sql`, adding three VALID constraints to `public.conversations` — `conversations_restaurant_id_id_key` UNIQUE `(restaurant_id, id)`, `conversations_channel_v1_check` CHECK `channel IN ('whatsapp','website')` and `conversations_safety_state_check` CHECK `is_safety_hold = false OR ownership_state IN ('SYSTEM_HOLD','HUMAN_ACTIVE','HUMAN_IDLE')` — with no M-1 or M-2 object and no data remediation. A local disposable-PostgreSQL proof reported **46 / 46 PASS** against that exact artifact. The proof and implementation are **BUILDER EVIDENCE ONLY** pending independent Quality exact-head review: M-0 is **not accepted, not merged, not applied, not production-deployed and not production proven**, and production application of `0107` has not occurred. Production access during M-0 implementation was ZERO — zero production queries, zero production writes, zero production DDL, zero production remediation and zero production deployments. Automatic Vercel Preview deployment(s) occurred for PR #571; these are non-production preview deployments and did not constitute production deployment or production database application of `0107`. Primary proof evidence is custodied at `scripts/proof-kiv12-m0-disposable-pg.py` and `docs/governance/KIV-12_M0_DISPOSABLE_PG_PROOF_2026-08-12.txt` for independent Quality verification. `M-1` / `KIV-13` remains downstream and must not begin until the M-0 gate is independently accepted and PM processes that verdict; `R-3` remains HARD STOP; `0105` remains reserved; `0106` remains reserved and BLOCKED; `0077` remains unapplied and unauthorized; and pilot remains NO-GO |
| 13 Aug 2026 | `KIV-13` / `CTRL-04` **M-1 ADDITIVE — SCOPE-1** built as a **stacked candidate** on `feat/kiv13-m1-additive-scope1` from the Quality-accepted `KIV-12` M-0 head `f20b56889dc354a411b0b46cd627b355fc732183`, under founder-approved `AGENT-WO-002A` and the founder-approved KIV-13 M1 Binding Adjudication of 13 August 2026. PR #571 remains draft and unmerged and `0107` is not production-applied. Revision 14 was supplied and independently verified at SHA-256 `f5eda59d06bdb4e72af183ab70deaec5dbb0041a02b6c5ba59bad21e456f6c37`, 156,719 bytes, 2,140 lines. The change carries one authorized migration candidate, `supabase/migrations/0108_kiv13_m1_additive_scope1.sql` (SHA-256 `915e0032da60e27d63d1547999a872c95540b0ed3db7c086fcde3c0a8f104bd2`), creating `kivo_control_owner`, its additive privileges, the seven preserved owner policies, `member_identity_versions` with `ux_miv_open_version`, three immutability triggers, four policies and a population-relative initialization, A0 `control_operations` with `ux_a0_evidence_core` and four evidence triggers, A2 `conversation_audit_failures` with `fk_a2_parent_core` and four evidence triggers, eight nullable A1 columns, `kv_control_result`, and `F1..F16` with SA1 EXECUTE separation — SA2 withdrawn and not implemented. It revokes nothing, changes no existing owner, forces no RLS on an existing table, adds no trigger to an existing table, adds no constraint to a populated table and writes no existing row. A local disposable-PostgreSQL proof reported **119 / 119 PASS**. The proof and implementation are **BUILDER EVIDENCE ONLY** pending independent Quality exact-head review: M-1 is **not accepted, not merged and not production-applied**. Production database actions were ZERO; automatic Vercel Preview deployment(s) for the pull request are non-production. `M-2` has not started; the A1 evidence triggers and A1 hardening remain M-5; `R-3` remains HARD STOP; `0105` reserved; `0106` reserved and BLOCKED; `0077` unapplied and unauthorized; pilot remains NO-GO |
| 13 Aug 2026 | `KIV-13` / `CTRL-04` M-1 **Quality rejection correction**. Quality returned QUALITY GATE REJECTED on five behavioural defects at head `3dddc7267ba31d34aff9361c0acb4b0517b35727`, which is preserved in history as evidence. All five are corrected on normal child commits: claimability restored to the 11 August adjudication's exact `CLAIMABLE_FROM` with `HUMAN_ACTIVE` idempotent-only; F5/F8/F9 actor authority; SB4 status write-through on every applied transition with nothing written on a no-op; F14's A0 replay moved before the row re-read and create-or-adopt; and one generic KIV11 for all ten non-creation operations so the API is not an existence oracle. Migration `0108` SHA-256 `06bc41f51c11f345b2d52240b5c69c32470b6fcb3e9554e8a6760492288e5f23`. The proof harness grew 119 → **190 assertions**, none weakened, with an exact F1..F16 name-and-argument-type inventory replacing the earlier weak check; the corrected candidate reports **190 / 190 PASS** from a clean committed tree. Recorded without changing M-1 scope: the **A1 event_type 8-vs-13 gap is a PRE-R3 GOVERNANCE / STAGE-ORDERING BLOCKER requiring FOUNDER/PM SPEC ADJUDICATION** — the live CHECK omits `HOLD_RELEASED`, `REOPENED`, `IDLE`, `TIMEOUT_RETURNED` and `STALE_ASSIGNEE_CLEARED`, A1-2 belongs to M-5 while callers convert at R-3, and `0108` does not widen the populated CHECK. M-1 is a **CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING** — not accepted, not merged, not production-applied. M-2 has not started; R-3 remains HARD STOP; production database actions ZERO; Vercel Preview non-production; pilot remains NO-GO |
| 13 Aug 2026 | `KIV-13` / `CTRL-04` M-1 **further Quality correction**. Five further defects corrected on normal child commits with both earlier heads preserved: F5/F8/F9 actor authority moved to **D2** so no unauthorized no-op can succeed; F14's actual fingerprint replay decision moved before any conversation re-read, create or adopt; the create_safety fingerprint corrected to the exact §16.2 field order including the previously omitted customer and channel; F13 restructured to resolve replay before reading conversation or configuration and to fingerprint all five expected timeout inputs; and governed fingerprint text **NFC-normalized** via the core `normalize(value, NFC)` built-in with no extension, object or sixth path. Migration `0108` SHA-256 `7e2e28018d20f0d9d5d609946ac643688d773d7c84e54fd0078d747e3c3e31c6`. Proof harness 190 → **235 assertions**, none weakened, **235 / 235 PASS** from a clean committed tree. No accepted correction regressed. `KIV-74` unchanged and separate — the A1 8-vs-13 gap remains an OPEN PRE-R3 GOVERNANCE / STAGE-ORDERING BLOCKER for founder/PM spec adjudication and `0108` does not widen the populated CHECK. M-1 is a **FURTHER-CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING** — not accepted, not merged, not production-applied. M-2 not started; R-3 HARD STOP; production database actions ZERO; Vercel Preview OCCURRED and is non-production; pilot NO-GO |
| 13 Aug 2026 | `KIV-13` / `CTRL-04` M-1 **concurrency and request-identity correction**. Two behavioural defects corrected on a normal child commit of the rejected head `a1f173feb2a3ec336f22c9bea902a5817974a410`, which is preserved as its direct parent along with the two earlier heads. **F13 lock ordering:** `kv_sys_control_timeout_return` now takes the governed conversation row lock (T6) with `SELECT ... FOR UPDATE` **before** the T7 expected-state and configuration validation, so validation and the transition decision are evaluated against the locked row and a stale timeout can never overwrite a fresh human claim or reassignment. **Authoritative request identity:** the §16.2 fingerprint fields absent from the universal core's governed signature — customer and channel for `create_safety_conversation`, the five expected inputs for `timeout_return` — are no longer reconstructed inside the core from mutable conversation and tenant state; F13 and F14 compute the canonical identity from the original caller inputs and hand it to the core transaction-locally, which consumes it once, clears it and refuses those two operations if it is absent or malformed. No governed public signature changed, no new object, extension or authority was introduced and no sixth path was added. Migration `0108` SHA-256 `8a435974b1b302223548b4aeadfa84ee0a11918a301d8698f7774a49f757a107`. Proof harness 235 → **251 assertions**, none weakened, **251 / 251 PASS** from the clean committed head `66e4e20a49fa11e0c80e592aa51c637337290682`, with deterministic concurrency tests exercising actual PostgreSQL lock and serialization behaviour — a real second session holding the row lock while F13 blocks and writes no A0 row, a fresh governed claim committing before a stale timeout that is then refused KIV21 with ownership intact — and both F13's and F14's stored A0 fingerprints checked against an independent SQL §16.2 oracle. Raw transcript custodied at `docs/governance/KIV-13_M1_DISPOSABLE_PG_PROOF_2026-08-13.txt` (SHA-256 `ac2c4448470e15c7b306de84e78aa134cb1a1b3ab5ae06504bf64a7b5abdc097`, 44,973 bytes) with the executable proof at `scripts/proof-kiv13-m1-disposable-pg.py` (SHA-256 `96dc5e14e8ea6adf84aa3ce6b4ad4717a06e6b52b41ca6e3847fa05b7df21556`). No accepted correction regressed. `KIV-74` unchanged and separate — the A1 8-vs-13 gap remains an OPEN PRE-R3 GOVERNANCE / STAGE-ORDERING BLOCKER for founder/PM spec adjudication and `0108` does not widen the populated CHECK. M-1 is a **CONCURRENCY-CORRECTED CANDIDATE / QUALITY RE-REVIEW PENDING** — not accepted, not merged, not production-applied and not production proven. M-2 not started; R-3 HARD STOP; `0105` reserved; `0106` reserved and BLOCKED; `0077` unapplied and unauthorized; production database actions ZERO; Vercel Preview OCCURRED and is non-production; pilot NO-GO |
| 14 Aug 2026 | `KIV-14` / `KIV-77` **0108 transactional-bootstrap source correction**. Production `0107` is applied and read back; production `0108` is **not applied** — its attempt failed closed and rolled back completely with `public.members` unchanged at N=20. Root cause verified on PostgreSQL 17: Supabase executes as `postgres` (`rolsuper=false`, `rolcreaterole=true`) and PostgreSQL 16+ records the automatic `CREATE ROLE` grant as ADMIN TRUE / SET FALSE / INHERIT FALSE, so the executor cannot `SET ROLE` the new owner and the first ownership transfer aborts with SQLSTATE 42501; schema `CREATE` alone does not resolve it. `0108` now carries section 1B (minimum temporary bootstrap: the role granted to the executor `WITH INHERIT TRUE, SET TRUE`, plus `CREATE` on schema `public`) and section 16 (removes both, then verifies and fails closed). Accepted M-1 final state is unchanged, and per the 14 August adjudication exactly one membership survives — the unremovable automatic record at ADMIN TRUE / SET FALSE / INHERIT FALSE granted by the bootstrap superuser — after which the executor provably cannot `SET ROLE kivo_control_owner` and inherits none of its object permissions. Corrected `0108` SHA-256 `7b24f4ea7f8c6ae56d6469cc998a71fcfb5cdd19f942c2701709a2f796701b92`; the historical accepted `8a435974…f757a107` remains immutable at `bf5f982b…`. Disposable PostgreSQL 17.11 proof applied by a non-superuser executor in one transaction: **288 / 288 PASS** from clean committed head `2738a70b0dcb894317b5a7cb8bb0f166fd2e9690`, covering the historical failure reproduction, failure-injection rollback at two points and full cleanup verification; transcript custodied at `docs/governance/KIV-14_0108_BOOTSTRAP_CORRECTION_PROOF_2026-08-14.txt`. Assertion history stated honestly: the historical combined zero-membership assertion WAS REPLACED — no blanket "nothing removed, skipped or weakened" claim is made. Inbound membership changed from zero to at most one (a relaxation); outbound membership remains zero; the one permitted inbound row must be the exact unusable automatic row; and the shape, bootstrap-superuser grantor, no-SET, no-INHERIT, SET-ROLE-denied and no-inherited-permission checks compensate for that relaxation. Source and disposable-test work only: no production access, no production retry, not merged. M-2/KIV-15 not started; KIV-74 open; R-3 HARD STOP; `0105` reserved; `0106` reserved and BLOCKED; `0077` unapplied and unauthorized; pilot remains NO-GO |
| 15 Aug 2026 | `KIV-77` **0108 bootstrap correction — cycle 2**, closing KIV-81 findings W1–W4. **W1**: `0108`'s section 16 check now verifies the grantor — any membership on `kivo_control_owner` recorded by a non-superuser grantor aborts the migration, since the one permitted record is the automatic `CREATE ROLE` grant made by the bootstrap superuser. **W2**: a new disposable PostgreSQL 17 test covers `PUBLIC` already holding `CREATE` on schema `public`, where the bootstrap-only grant cannot be cleanly withdrawn; the migration aborts on the governed fail-closed check and rolls back completely. **W3**: every “strictly stronger” claim is removed — accurately, the permitted inbound membership count changed from zero to at most one under the approved 14 August adjudication, a relaxation rather than a tightening, while `SET`/`INHERIT` remain forbidden and outbound membership remains forbidden entirely. **W4**: teardown is failure-safe and task-scoped — unique run tag, `0700` run directory owned by `postgres`, `0600` owner-only temp and injected SQL files, dynamic port, socket inside the run directory, and an `atexit` teardown registered before any cluster exists that removes every task-owned cluster, process, socket, PGDATA and injected file; verified out of band with an unusable `PGBIN`, leaving nothing behind. Corrected `0108` SHA-256 `d0e9a595399dcf60e653c92b05fd4d5174db3a6488200f8d0ab050fc1f9fb50b`. Proof **302 / 302 PASS** (from 288) on PostgreSQL 17.11 from clean committed head `9850a88b4fe6a01452d28b02ac68e7c28e4abdab`. Recorded limitation: a single surviving membership with a non-superuser grantor cannot be constructed, so the grantor guard is defence-in-depth alongside the count guard. Source and disposable-test work only: no production access, no production retry, PR #573 remains draft and unmerged. M-2/KIV-15 not started; KIV-74 open; R-3 HARD STOP; `0105` reserved; `0106` reserved and BLOCKED; `0077` unapplied and unauthorized; pilot remains NO-GO |
| 15 Aug 2026 | `KIV-87` **PR #573 Quality correction**, closing KIV-85 findings Q1–Q5 after `QUALITY GATE HARD STOP — EVIDENCE OR ENVIRONMENT GAP` (the Quality environment had PostgreSQL 16 only). **Q1**: `historical_0108.sql` was written `0644` against a blanket `0600` owner-only claim; it now uses the owner-only helper and an executable check verifies every task-created `.sql` file is `0600` and owned by `postgres`. **Q2**: Assertion history stated honestly: the historical combined zero-membership assertion WAS REPLACED — no blanket "nothing removed, skipped or weakened" claim is made. Inbound membership changed from zero to at most one (a relaxation); outbound membership remains zero; the one permitted inbound row must be the exact unusable automatic row; and the shape, bootstrap-superuser grantor, no-SET, no-INHERIT, SET-ROLE-denied and no-inherited-permission checks compensate for that relaxation. **Q3**: the whole-file transaction wrapper is recorded in the `0108` header as a mandatory fail-closed precondition and proven executably — the same file with the same injected failure leaves the role and a usable SET/INHERIT grant behind statement-by-statement, and leaves nothing under `psql --single-transaction`; `0108` carries no top-level BEGIN/COMMIT and must never be applied statement by statement. **Q4**: a separately Founder-authorized SELECT-only production preflight for PUBLIC schema CREATE is recorded as mandatory before any future production application, with the PostgreSQL 17 abort-and-rollback proof retained and no permission repair authorized. **Q5**: limitations kept explicit — the W1 grantor state cannot be isolated because the count guard fires first, the source-substring assertion is not full behavioural proof, and INHERIT is required by this candidate's current statement order rather than by every possible design. Corrected `0108` SHA-256 `8b3aa745f8e60178ebda7a9101fc4d654e252b8d27372924434d00345d59090c`; proof **315 / 315 PASS** on PostgreSQL 17.11 from clean committed head `d772ff449418076799ca78a388231c33f4a7f9b1`. Source and disposable-test work only: no production access, no production retry, no permission repair, PR #573 remains draft and unmerged. M-2/KIV-15 not started; KIV-74 open; R-3 HARD STOP; `0105` reserved; `0106` reserved and BLOCKED; `0077` unapplied and unauthorized; pilot remains NO-GO |
| 16 Aug 2026 | `KIV-126` **0108 member-identity and pre-existing role guard** (`KIV-124` P1). Section 16 now rejects any surviving membership on `kivo_control_owner` whose member is not the governed executor, ordered before the count guard so the failure names the offender; and a new section-1 check verifies unconditionally that the role — created here or pre-existing — carries only the governed safe attributes NOLOGIN/NOSUPERUSER/NOBYPASSRLS/NOCREATEDB/NOCREATEROLE/NOINHERIT/NOREPLICATION, naming any unsafe one. Both fail closed under the mandatory whole-file transaction wrapper. Corrected `0108` SHA-256 `00cd7b7fe2ee581df7b9d038301123db45a80962fe8a6ad3c0435e2893dea9ee`; disposable PostgreSQL 17.11 proof under the faithful non-superuser executor at **357 / 357 PASS** from clean committed head `04f73aec3d05091764295bce44acf0f395afd871`, adding an unrelated-member abort with complete rollback, all seven unsafe-attribute aborts with complete rollback, safe pre-existing role acceptance, and a second application that neither reinitializes nor rewrites MIV. All 315 previously accepted checks preserved, none skipped; the single W1 guard-label change is disclosed and is not a weakening. Transcript custodied at `docs/governance/KIV-126_0108_MEMBER_IDENTITY_ROLE_GUARD_PROOF_2026-08-16.txt`; historical transcripts untouched. Source and disposable-test work only: no production or Supabase access, no production retry, draft PR, not merged. M-2/KIV-15 not started; KIV-74 open; R-3 HARD STOP; production remains M-0 applied / M-1 not applied with members N=20; pilot remains NO-GO |
