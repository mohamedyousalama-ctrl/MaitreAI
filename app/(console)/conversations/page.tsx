...
function safetyHold(c: Conversation, orderNotes?: string): boolean {
...
function intentPill(c: Conversation): PillTone {
  if (isEscalated(c)) return { label: "تصعيد", bg: "rgba(192,73,47,.12)", fg: "#c0492f" };
...
export default function ConversationsPage() {
  ...
  const list = useMemo(() => {
    ...
    return conversations.filter((c) => {
      ...
      if (filter === "idle" && !(c.messages[c.messages.length - 1]?.sender === "customer" && !isEscalated(c))) return false;
  ...
  const aiSafe = hydrated ? conversations.filter((c) => c.owner === "ai" && !isEscalated(c)).length : 0;
...
function Thread({ c, onTakeover, onReturn, onSend, latestOrder }: {
  ...
  const humanOwns = c.owner === "human";
  const hold = safetyHold(c, latestOrder?.notes);
  const reason = c.escalationReason || (hold ? "حساسية + تأكيد مكوّنات" : isEscalated(c) ? "محتاجة تدخّل" : "");
    ...
    <div style={{ borderRadius: 16, background: "var(--kv-card)", border: "1px solid var(--kv-border)", boxShadow: "var(--kv-shadow-panel)", display: "grid", gridTemplateRows: "auto 1fr auto", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 16px", borderBottom: "1px solid #eef2f0", background: "rgba(255,255,255,.6)" }}>
        ...
        <div style={{ display: "flex", gap: 7 }}>
          ...
          <button onClick={() => onTakeover(c.id)} disabled={humanOwns} style={ghostBtn(humanOwns)}>إسناد للفريق</button>
          <button onClick={() => onTakeover(c.id)} disabled={humanOwns} style={primaryBtn(humanOwns)}>استلم وردّ</button>
        </div>
      </div>
  ...
  return (
    <div className="kv-scroll" style={{ overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <Card>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9, padding: 10, borderRadius: 12, background: humanOwns ? "rgba(192,73,47,.06)" : "rgba(14,159,110,.06)", border: `1px solid ${humanOwns ? "rgba(192,73,47,.16)" : "rgba(14,159,110,.18)"}` }}>
          <span className={humanOwns ? "kv-urgent" : "kv-pulse"} style={{ width: 8, height: 8, borderRadius: "50%", background: humanOwns ? "#c0492f" : "var(--kv-primary)" }} />
          <div style={{ fontSize: 11.5, fontWeight: 800, color: humanOwns ? "#a8412c" : "#0a8a5f" }}>{humanOwns ? "محوّلة للفريق" : "كريم نشط على المحادثة"}</div>
        </div>
        <button onClick={() => onTakeover(c.id)} disabled={humanOwns} style={{ ...primaryBtn(humanOwns), width: "100%", height: 40, marginTop: 10, gap: 8 }}>
          <ArrowLeft size={15} /> استلم المحادثة
        </button>
        {/* return-to-AI → deliberate chooser (never blind resume) */}
        <button onClick={() => setChooser((v) => !v)} disabled={!humanOwns} style={{ width: "100%", height: 36, marginTop: 8, border: "1px solid var(--kv-border)", borderRadius: 12, background: humanOwns ? "#fff" : "var(--kv-card-soft)", color: humanOwns ? "var(--kv-muted)" : "var(--kv-faint)", fontSize: 11.5, fontWeight: 800, fontFamily: "inherit", cursor: humanOwns ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
