...
// ============================================================================
// Kivo — الرئيسية (Home / Control). SPEC 08. The LAST console page: the
// operator's landing/router. Renders inside <ConsoleLayout> (.kv-console).
//
// Answers three questions, in order: is Karim working? what needs me right now?
// what's today's pulse? — then links into every page. It is a router + status
// board, NOT deep analytics (that's Insights).
//
// ROUTING: this lives at /dashboard inside (console), REPLACING the old
// (main)/dashboard MaitreAI page (deleted in this change — `D` in the diff). The
// /dashboard URL is preserved; the server-side post-login redirects already
// point here (lib/supabase/middleware.ts → /dashboard, auth/callback default
// next=/dashboard). LoginForm's client push is updated /orders → /dashboard.
//
...
//  • Orders to action  ← order-store (pending confirmation/payment)         LIVE
...
import type { Conversation, LocalOrder, OrderStatusKey } from "@/lib/types";
...
export default function HomePage() {
  ...
  const pulse = useMemo(() => {
    ...
    const convosToday = convosKnown ? conversations.filter((c) => { const t = convTime(c); return t != null && t >= startOfToday; }).length : 0;
    ...
    <div style={{ maxWidth: 1400, margin: "0 auto", color: "var(--kv-text)", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <section style={{ ...rNeeds.style }}>
        ...
        {needsTotal === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(248px,1fr))", gap: 12 }}>
            {/* Safety holds — highest priority, exact operational-hint-not-guarantee framing */}
            {safetyHolds > 0 && (
              <NeedCard
                tone="danger"
                icon={ShieldAlert}
                count={safetyHolds}
                title="تنبيه سلامة في محادثات"
                body="ده تنبيه تشغيلي للفريق — مش ضمان سلامة. راجعها فوراً وأكّد المكوّنات بنفسك قبل أي تأكيد."
                href="/conversations"
                cta="راجع المحادثات"
              />
            )}
            {escalations > 0 && (
              <NeedCard
                ...
                icon={Bell}
                count={escalations}
                title="محادثات محتاجة تدخّل"
                body="كريم حوّلها للفريق أو فيها حاجة محتاجة قرار بشري. استلمها وكمّل مع العميل."
                href="/conversations"
                cta="افتح المحادثات"
              />
            )}
            {ordersToAction > 0 && (
              <NeedCard
                tone="info"
                icon={ClipboardList}
                count={ordersToAction}
                title="طلبات محتاجة متابعة"
                body="طلبات لسه مستنية تأكيد أو دفع. اتأكّد منها عشان تكمّل للمطبخ."
                href="/orders"
                cta="افتح الطلبات"
              />
            )}
      ...
      <section style={{ ...rPulse.style }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          ...
          <PulseTile
            label="محادثات النهارده"
            value={pulse.ready && pulse.convosKnown ? toAr(pulse.convosToday) : "—"}
            icon={MessagesSquare}
            gathering={pulse.ready && !pulse.convosKnown}
          />
