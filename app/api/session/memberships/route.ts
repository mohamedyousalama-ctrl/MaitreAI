...
export async function GET() {
  ...
  // The user's memberships, with the restaurant name joined (admin bypasses RLS;
  // scoped strictly to this user's rows by user_id).
  ...
  const memberships = (rows ?? [])
    .map((r) => {
      ...
      const rest = Array.isArray((r as { restaurants?: unknown }).restaurants)
        ? (r as { restaurants: Array<{ name?: string }> }).restaurants[0]
        : (r as { restaurants?: { name?: string } }).restaurants;
      return {
