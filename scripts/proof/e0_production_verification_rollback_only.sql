...
BEGIN
  ...
  SELECT id INTO v_event_a1 FROM public.brain_record_channel_event(
    TENANT_A, v_env_a, 'whatsapp', 'whatsapp', 'customer_inbound',
  ...
  SELECT id INTO v_event_b1 FROM public.brain_record_channel_event(
    TENANT_B, v_env_b, 'whatsapp', 'whatsapp', 'customer_inbound',
  ...
  v_evidence := jsonb_build_array(jsonb_build_object(
    ...
    'disclosure_class', 'allergy',
    'matched_excerpt', MARK || ' allergy probe',
    'excerpt_start', 0,
    'excerpt_end', 30,
  ...
  BEGIN
    ...
    VALUES
      (TENANT_A, v_scan, 'customer_text', 'allergy', MARK || ' late child',
  ...
  BEGIN
    PERFORM public.brain_fail_ingress_safety_scan(
      ...
      MARK || '-rawhash-a3', MARK || '-scanner', 'database_unavailable', now());
