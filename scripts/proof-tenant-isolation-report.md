...
| `app/api/dev/tts-bakeoff/route.ts` | No | **No tenant is involved at all.** Reads no request field except two enum-like query parameters (`model`, `line`) and touches no tenant data, no database and no Supabase client of any kind. | Trustworthy, and it is listed here for completeness rather than because it is public — **it does not exist on the production domain.** The handler's FIRST act, before a parameter is read or the key is touched, is to 404 unless `VERCEL_ENV === "preview"`; the `NODE_ENV` clause beside it is deliberately redundant so a build outside Vercel (where `VERCEL_ENV` is unset) fails closed too. Vercel Authentication (SSO, currently on for everything except custom domains) is a SECOND lock and is treated as one — a protection setting is a dashboard toggle someone can flip, so the code never relies on it. It exists to measure ElevenLabs time-to-first-byte per model **inside the deployment**, because the alternative is copying `ELEVENLABS_API_KEY` onto a laptop or into a chat window to run the same measurement, and both are worse than the problem. The caller supplies **no text**: four fixed lines live in the module and are chosen by id, so this cannot be steered into the free text-to-speech oracle in our registered voice that `app/api/demo/speak/route.ts` exists to avoid being. The model id is validated against the account's own `/v1/models` response before anything is spent, so an unknown id is refused rather than billed. One model, one line, one request bounds the spend to a single synthesis and keeps every call far inside `maxDuration`. The key is read from the environment, never echoed — the only fact about it that ever leaves the handler is the boolean `keyPresent`. It writes **no** `agent_runs` row, which is a real gap and acceptable only because it cannot run in production: spend here is a deliberate operator action against a preview URL, not visitor traffic. **It calls a model KIV-313 has not accepted**, which is precisely why it must never be reachable where a customer is — the refusal guard in `lib/ai/tts/elevenlabs.ts` is untouched and this builds its own request rather than weakening it. Delete it once the model review has its numbers. |
