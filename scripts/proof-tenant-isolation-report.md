...
| `app/api/demo/greeting/route.ts` | Yes | **No tenant is involved at all.** Reads no request field beyond the Host header and an opaque session id from the query string, and touches no tenant data. | Trustworthy: the caller supplies **no text**. Public and unauthenticated **by design**, behind the same in-handler host gate as every other demo route. It answers ONE thing — a signed speech ticket for a FIXED opening sentence that lives in `lib/demo/call-greeting.ts` — so a call screen greets the visitor instead of opening onto a silent line. Controls: the greeting text is a server constant, never anything a caller sent, so this cannot be steered into a text-to-speech oracle; the ticket is minted through the same shared voice gate as a turn, so a greeting a turn would refuse to speak is refused here too; it is minted **only when the voice is actually audible**, so a visitor whose voice is off never books a synthesis for a screen about to say "unavailable"; the session id is echoed from the query string and **never trusted** — it binds the ticket to the session that will redeem it, so a wrong value makes the greeting unplayable and nothing else; and the per-IP allowance is the **turn cap**, not the capability probe's 4× allowance, because every hit here books a paid synthesis. `no-store`: a signed, one-minute, session-bound ticket has no business in any cache. **It writes exactly one row, and it must**: the ticket it mints is redeemed at `/api/demo/speak`, which treats a first fetch as already paid for by whoever minted it — so this route buys a real synthesis, and an earlier version of this row recorded "no write of any kind occurs" as a virtue while that charge reached no ledger and `lib/monitoring/sweep.ts` (which sums `agent_runs.cost_usd` for the daily spend alert) could not see it. The write is a single `agent_runs` row with the mint's own cost, model and adapter, through `mustWrite({exactRows:1})`, and it is deliberately **not** fail-closed — a ledger outage must not take the greeting down. No tenant data is read or written; the row is cost accounting against the synthetic demo tenant. **Deliberately not part of `demo/capabilities`**, which promises one boolean and no voice id: a ticket payload carries the registered voice id, and the probe's wider allowance is granted on the stated grounds that it does no I/O. |
| `app/api/demo/silent/route.ts` | No | **No tenant is involved at all.** Reads no request field except a closed enum and three clamped integers, and touches no tenant data, no database, and no Supabase client. | Trustworthy, and it is the cheapest route in this table: it BUYS NOTHING — no synthesis, no model call, no write — so there is no spend to guard and it deliberately does not consume the demo's durable turn allowance, which would make a diagnostic compete with the thing it diagnoses. Public and unauthenticated **by design**, behind the same in-handler host gate as every other demo route. It exists because the call screen already works out why a reply made no sound — a rejected play(), a decode error, a stall, an early barge — and then wrote that to a browser console on someone else's phone, so a founder's "he was silent on the second message" could not be answered from server logs showing the audio synthesized in 918ms. Controls: the `reason` is matched against a CLOSED SET and anything else is logged as `unknown_reason`, never echoed — a free-text field on a public endpoint is a log-injection primitive and an unbounded log bill; the three numerics are truncated and clamped, so a caller cannot widen a log line; `detail` keeps only the one piece of provider vocabulary worth having (a DOMException name such as `NotAllowedError`, which separates an autoplay policy from a decode failure) and is allow-listed to word characters and clipped to 40, so it can carry neither a newline nor a payload; and the per-IP limiter bounds the volume. A refused report answers 200 — a diagnostic that fails loudly on a broken call teaches the caller to stop sending them. |
| `app/api/faysal/turn/route.ts` | Yes | **Not determined from the caller at all.** The clinic is the single seeded Al Wattan demo tenant, resolved server-side; no clinic id, site id or member id is ever read from the request body, path or headers. The body carries exactly two fields — a length-capped `text` and an opaque signed `sessionId`. | Trustworthy: there is nothing for a caller to influence. Public and unauthenticated **by design** — it is the page a founder opens in front of a client, and it is in `PUBLIC_PREFIXES` for that reason. This is the FAYSAL (health) vertical, so the isolation that matters is twofold: it touches only `health_*` tables, every one of which is `clinic_id`-scoped with RLS enabled AND forced, and it imports nothing from the restaurant domain — the seam is the point, and `health_members` is deliberately empty so no authenticated user can read a health row at all; the demo runs server-side, as the Kivo demo does. Hardened the same way `app/api/demo/turn` is: input length-capped, a per-IP limiter and a durable daily spend ceiling (`_engine/guard.ts`), and the reply is composed from frozen strings and tool results rather than echoed. TWO CONTROLS ARE SPECIFIC TO A CLINIC: the red-flag rail runs on raw inbound BEFORE any model call and, on a hit, returns a frozen string with no tool set and no booking path — and a triage hold makes that verdict survive the next turn, so an emergency cannot be talked back into a booking; and every price and every confirmation carries the demo label, because a screenshot of a confirmed appointment must never be mistakable for a real Al Wattan booking. |
| `app/api/faysal/reset/route.ts` | No | **No tenant is involved at all.** It mints a fresh signed session envelope and returns it; it reads no tenant identifier and writes nothing. | Trustworthy, and nearly as cheap as `app/api/demo/silent`: it BUYS NOTHING — no model call, no synthesis, no database write — so there is no spend to guard. It exists so a founder can restart cleanly mid-demo rather than reloading and losing the thread, which matters when the next thing being demonstrated is the emergency rail and the previous session is still holding a triage lock. Public and unauthenticated by design, per-IP limited, and because the session is a signed opaque envelope a caller cannot forge one that resumes someone else's conversation or clears a triage hold that a red flag set. |
